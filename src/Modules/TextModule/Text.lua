...
-- Classes
local Sprite = require(script.Parent.lib.TextSprite)

-- Modules
local Util = require(script.Parent.lib.Util)

-- Variables
local Unicode = require(script.Parent.lib.Unicode)
...
function Text.new(font_name, automatic_update)
	local font_folder = script.Parent.Fonts:FindFirstChild(font_name)
...
function Text:SetTransparency(value)
	for i = 1, #self.Sprites do
		if self.Sprites[i].Instance then
			self.Sprites[i].Instance.ImageTransparency = value
		end
	end
end
