...
function DialogModule.new(npcName, npc, prompt, animation)
	...
	self.dialogOption = 1
	self.npcGui = self.npc:WaitForChild("Head"):WaitForChild("Info")
	self.active = false
	self.talking = false
	self.prompt = prompt
	...
	local heartbeatConnection = runService.Heartbeat:Connect(function()
		...
		if self.talking then
			...
			self.npcGui.StudsOffset = Vector3.new(0,math.sin(frameCount/120)/12 + 1.55,0)
		end
	end)
	...
	task.spawn(function()
		...
		for i, response in ipairs(dialog.responses) do
			...
			local enterCon = option.MouseEnter:Connect(function()
				tweenService:Create(option.text,TweenInfo.new(0.1,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Rotation = -1}):Play()
				END_TICK_SOUND:Play()
			end)

			local leaveCon = option.MouseLeave:Connect(function()
				tweenService:Create(option.text,TweenInfo.new(0.1,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Rotation = 0}):Play()
			end)
...
function DialogModule:showGui()
	...
	self.npcGui.dialog.UIStroke.Transparency = 0.87
...
function DialogModule:hideGui(exitQuip, notActuallyAnExitQuip)
	...
	if exitQuip then
		...
		dialogObject.UIStroke.Transparency = 0.87
	...
	task.spawn(function()
		...
		self.npcGui.name.UIStroke.Transparency = 0.87
		self.npcGui.arrow.TextTransparency = 0
		self.npcGui.arrow.UIStroke.Transparency = 0.87
